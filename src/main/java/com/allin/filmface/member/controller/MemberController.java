...
public class MemberController {
    ...
    public ResponseEntity<ResponseDTO> findMemberByToken(@PathVariable String socialCode) {
        ...
        Map<String, Object> responseMap = new HashMap<>();
        responseMap.put("member", guestMember);

        return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "비회원 조회 성공", responseMap));
    }

    @ApiOperation("게스트 프로필 수정")
    @PutMapping("/guest/{socialCode}/profile")
    public ResponseEntity<ResponseDTO> updateProfile(@PathVariable String socialCode,
                                                     @RequestBody GuestDTO guestDTO) {

        GuestDTO guestProfile = memberService.guestprofile(socialCode, guestDTO);
    ...
    @DeleteMapping("/guest/{socialCode}")
    public ResponseEntity<?> deleteGuest(@PathVariable String socialCode) {

        memberService.deleteGuest(socialCode);
